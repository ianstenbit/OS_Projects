  ...
  for(int i = 0; i < PAGE_REF_STRING_LENGTH; i++){
    ...
    if(!contains(FIFO_memory, page_ref[i], max_frames)){
      ...
      if(DEBUG){
        ...
        printf("\tMemory before swap: [");
        for(int j = 0; j < max_frames; j++) printf("%i,", (FIFO_memory[j] == 16843009)? -1 : FIFO_memory[j]);
        printf("\b]\n");
      }
    ...
    if(!contains(LRU_memory, page_ref[i], max_frames)){
      ...
      if(DEBUG){
        ...
        printf("\tMemory before swap: [");
        for(int j = 0; j < max_frames; j++) printf("%i,", (LRU_memory[j] == 16843009)? -1 : LRU_memory[j]);
        printf("\b]\n");
      }
    ...
    if(!contains(OPT_memory, page_ref[i], max_frames)){
      ...
      if(DEBUG){
        ...
        printf("\tMemory before swap: [");
        for(int j = 0; j < max_frames; j++) printf("%i,", (OPT_memory[j] == 16843009)? -1 : OPT_memory[j]);
        printf("\b]\n");
      }
